...
(function () {
  ...
  _this.init = function (setupDefaultItems, params) {
    ...
    if (setupDefaultItems) {
      this.layersFolder = panel.addFolder('Seleção de Camadas');
      if (Q3D.Config.gui.customPlane) this.customPlaneFolder = panel.addFolder('Camada Personalizada');
  ...
  _this.initLayersFolder = function (scene) {
    ...
    for (var layerId in mapLayers) {
      ...
      folder.add(params.lyr[layerId], 'v').name('Visível').onChange(visibleChanged);
      folder.add(params.lyr[layerId], 'o').min(0).max(1).name('Opacidade').onChange(opacityChanged);
  ...
  _this.addAnimationFolder = function () {
    ...
    var btn, folder = panel.addFolder('Animação');
